...
export class HomeComponent implements OnInit {
  ...
  constructor(
    ...
    this._metaTagsService.setMetaTags({
      name: 'description',
      content: 'description da'
    });
    this._presentationService.step$.subscribe(val => {
      val === numberOfStepsOnPresentation + 1 && this.slide !== val
...
// tslint:disable-next-line: no-unused-expression
  ...
  ngOnInit() {
    if (isPlatformBrowser(this._platformId)) {
      const mc = new Hammer(this.home.nativeElement);
      mc.get('swipe').set({
  ...
  mouseFinish(event, reverse) {
    if (this.section === 4 && event.target.offsetParent.id === 'awards-list') {
      return false;
    }
    ...
    if (this.slide < numberOfStepsOnPresentation + 1) {
      ...
      if (event.deltaY > 0) {
        this.home.nativeElement.children.length - 1 > this.section
          ? this.section++
          : '';
      } else {
        this.section > 0
          ? this.section--
          : this._presentationService.setStep(numberOfStepsOnPresentation);
      }
      const currentElement = this.home.nativeElement.children[this.section];
      this.home.nativeElement.scroll({
        top: currentElement.offsetTop,
        behavior: 'smooth'
      });

      // tslint:disable-next-line: no-unused-expression
      this.section + 1 <= this.home.nativeElement.children.length &&
